...
def windows():
    ...
    commands.append("setup-x86_64.exe --no-desktop --no-shortcuts --no-startmenu --quiet-mode --wait --packages wget curl tar p7zip git openssh sed vim tmux zsh fontconfig fontforge ghostscript make automake cmake gcc-core gcc-g++")
...
def linux():
    ...
    commands.append("sudo apt-get install tmux zsh git curl wget p7zip-full p7zip-rar build-essential vim upower ffmpeg fontconfig vim-gnome network-manager file-roller software-properties-gtk baobab gnome-system-monitor gnome-disk-utility -y")
    commands.append("rm -rf ~/dotfile")
    commands.append("rm -rf ~/.oh-my-zsh")
    commands.append("git clone --depth=1 https://github.com/j16180339887/dotfile.git ~/dotfile")
    commands.append("git clone --depth=1 https://github.com/robbyrussell/oh-my-zsh.git ~/.oh-my-zsh")
