...
InstallPIP()
{
  ...
  echo y | $SUDO pip install --upgrade https://github.com/l1ving/youtube-dl/archive/master.zip
...
InstallMiniconda()
{
  ...
  echo y | ~/Miniconda3/bin/pip install --upgrade https://github.com/l1ving/youtube-dl/archive/master.zip
...
#   $SUDO systemctl enable reconnect.service
  if [[ $(command -v apt) ]]; then
    ...
    app-fast -S --noconfirm --needed p7zip base-devel command-not-found nano ffmpeg atool fontconfig traceroute mtr nmap openssl net-tools iproute2 bind python3 wget tig upx htop
  fi
...
#   pacman -S mingw-w64-x86_64-toolchain --noconfirm
  ...
  ~/dotfile/app-fast/app-fast full-upgrade -y --assume-yes
  ~/dotfile/app-fast/app-fast install -y --assume-yes man vim micro git tig zsh fish tmux curl aria2 file tar wget bash-completion htop openssh mosh grep sed gawk python ffmpeg p7zip
