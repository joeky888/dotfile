...
InstallDotfileCygwin()
{
  ...
  cygstart --action=runas cmd.exe /c mklink "%USERPROFILE%\Documents\WindowsPowerShell\Microsoft.PowerShell_profile.ps1" "%USERPROFILE%\dotfile\powershell\profile.ps1"
  cd $Home/dotfile && git pull origin master && git submodule update --init --recursive --remote --merge && git submodule foreach git pull origin master && cd -
...
#   export SUDO='cygstart --action=runas "$@" '
  ...
  apt-cyg install openssh rsync bind-utils bash-completion tig tar
