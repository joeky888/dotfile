...
#     Disable byobu because if it breaks colors of vim
    if [ $(command -v byobu) ] ; then
      exec byobu
       ...
       [[ $(tmux ls) ]] && exec tmux attach || exec tmux
    fi
  fi
...
#     PROMPT="%B%F{red}%n%B%F{yellow}@%B%F{green}%M %{$reset_color%}\n➜ %B%F{blue}%~"${NEWLINE_NO_OMZ}"%{$reset_color%}$ "
  ...
  function update_prompt() {
    ...
    PROMPT="╭─%K{red}%F{white} %n %K{yellow} %K{green} %M %B%F{yellow}%K{blue} %~ %F{yellow}%K{default} $(git_branch_info) "${NEWLINE_NO_OMZ}"%K{default}%F{default}╰─> "
  }
  update_prompt
  ...
  if [ $(command -v tput) ]; then
    ...
    export PS1="╭─${COLOR_BG_RED} \u ${COLOR_BG_YELLOW} ${COLOR_BG_GREEN} \h ${COLOR_BG_RESET} ${COLOR_BG_BLUE}${COLOR_YELLOW} \w ${COLOR_BG_RESET}${COLOR_RESET}\n╰─\$ "
  fi
fi
