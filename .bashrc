...
export DOWNLOADER_ARGUMENTS="-c --file-allocation=falloc --max-tries=0 --max-concurrent-downloads=16 --max-connection-per-server=16 --split=16 --min-split-size=1M" # Bypy
...
alias youtube-dl='youtube-dl --write-sub --ignore-errors --external-downloader aria2c --external-downloader-args "-c --file-allocation=falloc --max-tries=0 --max-concurrent-downloads=16 --max-connection-per-server=16 --split=16 --min-split-size=1M"'
...
alias aria2c='aria2c -c --file-allocation=falloc --max-tries=0 --max-concurrent-downloads=16 --max-connection-per-server=16 --split=16 --min-split-size=1M'
...
if [[ "$OSTYPE" == "linux-gnu" ]]; then # Ubuntu
  ...
  upgradeChoco() { cygstart --action=runas choco upgrade all -y ;}
