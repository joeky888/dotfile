...
export PLAYER_ARGUMENTS="--cache=yes --demuxer-cache-dir=/tmp --cache-on-disk=yes --ytdl-raw-options=sponsorblock-mark=all,embed-metadata=,no-check-certificate=,concurrent-fragments=8,yes-playlist=,hls-prefer-native=,ignore-errors=,write-auto-sub=,write-sub=,sub-lang='(en|zh).*'"
