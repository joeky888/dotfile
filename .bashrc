...
export PLAYER_ARGUMENTS="--cache=yes --cache-dir=/tmp --cache-on-disk=yes --force-seekable=yes --ytdl-raw-options=no-check-certificate=,concurrent-fragments=8,yes-playlist=,hls-prefer-native=,ignore-errors=,write-auto-sub=,write-sub=,sub-lang='(en|zh).*'"
...
export STREAMLINK_ARGUMENTS="--loglevel debug --verbose-player --player-no-close --stream-segment-threads 10 --twitch-low-latency --http-no-ssl-verify --title '{title}' --stream-segment-attempts 1000 --stream-segment-timeout 10 --stream-segment-attempts 1000 --stream-segment-timeout 10 --retry-open 10 --retry-max 10 --retry-streams 1"
...
if hash yt-dlp 2>/dev/null >/dev/null; then
  ...
  [ $(command -v curl) ] && alias curl='curl --retry 0 --connect-timeout 10 --max-time 10 --retry-delay 0 --retry-max-time 20 --compressed -H "Accept-Encoding: gzip,br,deflate" --user-agent "$(yt-dlp --dump-user-agent)" -LC - '
  [ $(command -v curl3) ] && alias curl3='curl3 --http3 --retry 0 --connect-timeout 10 --max-time 10 --retry-delay 0 --retry-max-time 20 --compressed -H "Accept-Encoding: gzip,br,deflate" --user-agent "$(yt-dlp --dump-user-agent)" -LC - '
  ...
  [ $(command -v curl) ] && alias curl='curl --retry 0 --connect-timeout 10 --max-time 10 --retry-delay 0 --retry-max-time 20 --compressed -H "Accept-Encoding: gzip,br,deflate" -LC - '
  [ $(command -v curl3) ] && alias curl3='curl3 --http3 --retry 0 --connect-timeout 10 --max-time 10 --retry-delay 0 --retry-max-time 20 --compressed -H "Accept-Encoding: gzip,br,deflate" -LC - '
...
dnslookup-adguard() { for rrtype in A AAAA CNAME NS HTTPS MX TXT SRV; do RRTYPE=$rrtype dnslookup $1 https://dns.adguard.com/dns-query | sed -e 's/^;; opcode: QUERY, status: //' -e 's/, id: .*$//' -e 's/^dnslookup.*//' -e 's/^;; flags.*//' -e 's/^;; .* SECTION://' -e '/^$/d'; done }
upgradeDotfile() {
