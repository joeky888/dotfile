...
mpv-fast-1080-sub() { mpv --cache=yes --demuxer-cache-dir=/tmp --cache-on-disk=yes --ytdl-raw-options=no-check-certificate=,concurrent-fragments=8 --audio-file=$(command yt-dlp --get-url -f 'bestaudio[protocol^=m3u]' $1) $(command yt-dlp --write-sub --write-auto-sub --sub-format vtt --print %(requested_subtitles.:.url)j --sub-langs "en.*,zh.*" $1 | jq -r '[.[] | "--sub-file=" + .] | join(" ")') $(command yt-dlp --get-url -f 'bestvideo[height<=1080][protocol^=m3u][vcodec!^=av01]' $1) --force-media-title="$(yt-dlp --print '%(title)s - %(upload_date>%Y/%m/%d)s' $1)" ;}
mpv-fast-1080-sub-en() { mpv --cache=yes --demuxer-cache-dir=/tmp --cache-on-disk=yes --ytdl-raw-options=no-check-certificate=,concurrent-fragments=8 --audio-file=$(command yt-dlp --get-url -f 'bestaudio[protocol^=m3u]' $1) --sub-file=$(command yt-dlp --write-sub --write-auto-sub --sub-format vtt --print %(requested_subtitles.en.url)s --sub-langs "en.*" $1) $(command yt-dlp --get-url -f 'bestvideo[height<=1080][protocol^=m3u][vcodec!^=av01]' $1) --force-media-title="$(yt-dlp --print '%(title)s - %(upload_date>%Y/%m/%d)s' $1)" ;}
mpv-fast-1080-sub-zh() { mpv --cache=yes --demuxer-cache-dir=/tmp --cache-on-disk=yes --ytdl-raw-options=no-check-certificate=,concurrent-fragments=8 --audio-file=$(command yt-dlp --get-url -f 'bestaudio[protocol^=m3u]' $1) --sub-file=$(command yt-dlp --write-sub --write-auto-sub --sub-format vtt --print %(requested_subtitles.zh.url)s --sub-langs "zh.*" $1) $(command yt-dlp --get-url -f 'bestvideo[height<=1080][protocol^=m3u][vcodec!^=av01]' $1) --force-media-title="$(yt-dlp --print '%(title)s - %(upload_date>%Y/%m/%d)s' $1)" ;}
