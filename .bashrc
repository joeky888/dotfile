...
dnslookup-google() { for rrtype in A AAAA CNAME HTTPS MX TXT; do RRTYPE=$rrtype dnslookup $1 https://dns.google/dns-query | sed -e 's/^;; opcode: QUERY, status: //' -e 's/, id: .*$//' -e 's/^dnslookup.*//' -e 's/^;; flags.*//' -e 's/^;; .* SECTION://' -e '/^$/d' ; done }
dnslookup-cloudflare() { for rrtype in A AAAA CNAME HTTPS MX TXT; do RRTYPE=$rrtype dnslookup $1 https://cloudflare-dns.com/dns-query | sed -e 's/^;; opcode: QUERY, status: //' -e 's/, id: .*$//' -e 's/^dnslookup.*//' -e 's/^;; flags.*//' -e 's/^;; .* SECTION://' -e '/^$/d'; done }
dnslookup-adguard() { for rrtype in A AAAA CNAME HTTPS MX TXT; do RRTYPE=$rrtype dnslookup $1 https://dns.adguard.com/dns-query | sed -e 's/^;; opcode: QUERY, status: //' -e 's/, id: .*$//' -e 's/^dnslookup.*//' -e 's/^;; flags.*//' -e 's/^;; .* SECTION://' -e '/^$/d'; done }
upgradeDotfile() {
