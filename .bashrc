...
mpv-fast-4k() { mpv --cache=yes --demuxer-cache-dir=/tmp --cache-on-disk=yes --ytdl-raw-options=no-check-certificate=,concurrent-fragments=8 --audio-file=$(command yt-dlp --get-url -f 'bestaudio[protocol^=m3u]' $1) $(command yt-dl --get-url -f 'bestvideo[height<=2160][protocol^=m3u][vcodec!^=av01]' $1) --force-media-title="$(yt-dlp --print '%(title)s - %(upload_date>%Y/%m/%d)s' $1)" ;}
...
mpv-fast-1080p() { mpv --cache=yes --demuxer-cache-dir=/tmp --cache-on-disk=yes --ytdl-raw-options=no-check-certificate=,concurrent-fragments=8 --audio-file=$(command yt-dlp --get-url -f 'bestaudio[protocol^=m3u]' $1) $(command yt-dlp --get-url -f 'bestvideo[height<=1080][protocol^=m3u][vcodec!^=av01]' $1) --force-media-title="$(yt-dlp --print '%(title)s - %(upload_date>%Y/%m/%d)s' $1)" ;}
