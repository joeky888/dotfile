...
export PLAYER_FASTARG_1080="--ytdl-format='best[height<=1080][protocol^=m3u8]/bv[height<=1080][vcodec!^=av01][protocol^=m3u]+ba[protocol^=m3u]/bv[height<=1080][vcodec!^=av01]+ba/best[height<=1080]'"
export PLAYER_FASTARG_2K="--ytdl-format='best[height<=1440][protocol^=m3u8]/bv[height<=1440][vcodec!^=av01][protocol^=m3u]+ba[protocol^=m3u]/bv[height<=1440][vcodec!^=av01]+ba/best[height<=1440]'"
export PLAYER_FASTARG_4k="--ytdl-format='best[height<=2160][protocol^=m3u8]/bv[height<=2160][vcodec!^=av01][protocol^=m3u]+ba[protocol^=m3u]/bv[height<=2160][vcodec!^=av01]+ba/best[height<=2160]'"
export IINA_ARGUMENTS="--mpv-cache=yes --mpv-demuxer-cache-dir=/tmp --mpv-cache-on-disk=yes --mpv-ytdl-raw-options=sponsorblock-mark=all,no-check-certificate=,concurrent-fragments=8,yes-playlist=,hls-prefer-native=,ignore-errors=,write-auto-sub=,write-sub=,sub-lang='(en|zh).*'"
...
alias upgradeYtdlp='pip3 install --upgrade --force-reinstall --no-cache-dir https://github.com/yt-dlp/yt-dlp/archive/master.zip https://github.com/yt-dlp/ejs/archive/master.zip'
...
alias upgradeYkdl='pip3 install --upgrade --force-reinstall --no-cache-dir https://github.com/SeaHOH/ykdl/archive/master.zip'
...
alias streamlink-mpv-best="streamlink $STREAMLINK_ARGUMENTS --player 'mpv' --player-arg '$PLAYER_ARGUMENTS' --title "{url}" --default-stream best"
alias streamlink-mpv-4kp60="streamlink $STREAMLINK_ARGUMENTS --player 'mpv' --player-arg '$PLAYER_ARGUMENTS' --default-stream 2160p60"
alias streamlink-mpv-4k="streamlink $STREAMLINK_ARGUMENTS --player 'mpv' --player-arg '$PLAYER_ARGUMENTS' --default-stream 2160p"
alias streamlink-mpv-2kp60="streamlink $STREAMLINK_ARGUMENTS --player 'mpv' --player-arg '$PLAYER_ARGUMENTS' --default-stream 1440p60"
alias streamlink-mpv-2k="streamlink $STREAMLINK_ARGUMENTS --player 'mpv' --player-arg '$PLAYER_ARGUMENTS' --default-stream 1440p"
alias streamlink-mpv-1080p60="streamlink $STREAMLINK_ARGUMENTS --player 'mpv' --player-arg '$PLAYER_ARGUMENTS' --default-stream 1080p60"
alias streamlink-mpv-1080="streamlink $STREAMLINK_ARGUMENTS --player 'mpv' --player-arg '$PLAYER_ARGUMENTS' --default-stream 1080p"
alias streamlink-mpv-720p60="streamlink $STREAMLINK_ARGUMENTS --player 'mpv' --player-arg '$PLAYER_ARGUMENTS' --default-stream 720p60"
...
dnslookup-cloudflare() { for rrtype in A AAAA CNAME NS HTTPS MX TXT SRV; do RRTYPE=$rrtype dnslookup $1 https://cloudflare-dns.com/dns-query | sed -e 's/^;; opcode: QUERY, status: //' -e 's/, id: .*$//' -e 's/^dnslookup.*//' -e 's/^;; flags.*//' -e 's/^;; .* SECTION://' -e '/^$/d'; done }
dnslookup-adguard() { for rrtype in A AAAA CNAME NS HTTPS MX TXT SRV; do RRTYPE=$rrtype dnslookup $1 https://dns.adguard-dns.com/dns-query | sed -e 's/^;; opcode: QUERY, status: //' -e 's/, id: .*$//' -e 's/^dnslookup.*//' -e 's/^;; flags.*//' -e 's/^;; .* SECTION://' -e '/^$/d'; done }
upgradeDotfile() {
...
if [[ -n "$ZSH_VERSION" ]]; then # Zsh
  ZSH_THEME=""
#   plugins=(git docker docker-compose golang rust deno bun) # Disable all plugins to load faster
  ...
  if [ -n "$POWERLEVEL10K_PATH" ]; then # powerlevel10k.zsh-theme is available
    ...
    export POWERLEVEL9K_COMMAND_EXECUTION_TIME_FORMAT='d h m s'
