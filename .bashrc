...
if hash youtube-dl 2>/dev/null >/dev/null; then
  ...
  [ $(command -v curl) ] && alias curl='curl --retry 0 --connect-timeout 10 --max-time 10 --retry-delay 0 --retry-max-time 20 --compressed -H "Accept-Encoding: gzip,compress,deflate" --user-agent "$(youtube-dl --dump-user-agent)" -LC - '
  ...
  [ $(command -v curl) ] && alias curl='curl --retry 0 --connect-timeout 10 --max-time 10 --retry-delay 0 --retry-max-time 20 --compressed -H "Accept-Encoding: gzip,compress,deflate" -LC - '
...
alias youtube-dl-240="youtube-dl -f 'bestvideo[height<=240][fps<=30][ext=mp4,vcodec!=AV1]+bestaudio/best'"
alias youtube-dl-360="youtube-dl -f 'bestvideo[height<=360][fps<=30][ext=mp4,vcodec!=AV1]+bestaudio/best'"
alias youtube-dl-480="youtube-dl -f 'bestvideo[height<=480][fps<=30][ext=mp4,vcodec!=AV1]+bestaudio/best'"
alias youtube-dl-720="youtube-dl -f 'bestvideo[height<=720][fps<=30][ext=mp4,vcodec!=AV1]+bestaudio/best'"
alias youtube-dl-1080="youtube-dl -f 'bestvideo[height<=1080][ext=mp4,vcodec!=AV1]+bestaudio/best'"
...
alias mpv-1080="mpv --ytdl-format='bestvideo[height<=1080][ext=mp4,vcodec!=AV1]+bestaudio/best' $PLAYER_ARGUMENTS"
alias mpv-720="mpv --ytdl-format='bestvideo[height<=720][fps<=30][ext=mp4,vcodec!=AV1]+bestaudio/best' $PLAYER_ARGUMENTS"
alias mpv-480="mpv --ytdl-format='bestvideo[height<=480][fps<=30][ext=mp4,vcodec!=AV1]+bestaudio/best' $PLAYER_ARGUMENTS"
alias mpv-mute="mpv --mute=yes --ytdl-format='bestvideo[height<=720][fps<=30][ext=mp4,vcodec!=AV1]+bestaudio/best' $PLAYER_ARGUMENTS"
alias mpv-loop="mpv --loop --ytdl-format='bestvideo[height<=720][fps<=30][ext=mp4,vcodec!=AV1]+bestaudio/best' $PLAYER_ARGUMENTS"
...
alias mpv-1080-auto-sub="mpv --video-aspect-override=16:9 --ytdl-format='bestvideo[height<=1080][ext=mp4,vcodec!=AV1]+bestaudio/best' ${PLAYER_ARGUMENTS},write-auto-sub=,write-sub=,sub-lang=en"
alias mpv-1080-ontop="mpv --ontop --ytdl-format='bestvideo[height<=1080][ext=mp4,vcodec!=AV1]+bestaudio/best' $PLAYER_ARGUMENTS"
...
if [[ -n "$ZSH_VERSION" ]]; then # Zsh
  ZSH_THEME=""
  plugins=(git docker docker-compose adb golang rust cargo)
  ...
  if [ -n "$POWERLEVEL10K_PATH" ]; then # powerlevel10k.zsh-theme is available
    ...
    export POWERLEVEL9K_TIME_BACKGROUND=42 # Green time
